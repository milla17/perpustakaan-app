...
while ($row = $res1->fetch_assoc()) {
  ...
  ';
  $output .= ambil_reply($connection, $row["komentar_id"]);
}

echo json_encode([$output]);


function ambil_reply($connection, $parent_id = 0, $marginleft = 0){
  $output='';
  ...
  if($count > 0){
    while ($row = $res1->fetch_assoc()) {
      ...
      ';
      $output .= ambil_reply($connection, $row["komentar_id"], $marginleft);
    }
  }
